...
@router.callback_query(F.data.startswith('attractions_action_'))
async def trip_action_tab(callback: CallbackQuery,
    ...
    for i, location in enumerate(trip_data['locations'], start=1):
        builder.row(InlineKeyboardButton(text=f"{i}. {location['name']}",
                                         callback_data=f'sight_show_{location["id"]}'))
...
@router.callback_query(F.data.startswith('sight_show_'))
async def show_sight_forecast(callback: CallbackQuery,
                                  state: FSMContext,
                                  backend: BackendService,
    ...
    response = await backend.places_service.get_sights(location['lng'], location['lat'], [
        16000, 10002, 10004, 10009, 10016, 10027, 10035, 10044, 10055, 10058
    ], custom_params='&fields=name,geocodes,description,tel,website,hours,rating,distance,categories,location')
