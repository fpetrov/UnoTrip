...
@router.callback_query(F.data.startswith('hotel_action_'))
async def hotel_action_tab(callback: CallbackQuery,
                          state: FSMContext,
    ...
    for i, location in enumerate(trip_data['locations'], start=1):
        builder.row(InlineKeyboardButton(text=f"{i}. {location['name']}",
                                         callback_data=f'hotel_show_{location["id"]}'))
...
@router.callback_query(F.data.startswith('hotel_show_'))
async def show_hotels(callback: CallbackQuery,
                              state: FSMContext,
                              backend: BackendService,
