...
@router.callback_query(F.data.startswith('car_rent_action_'))
async def trip_action_tab(callback: CallbackQuery,
    ...
    for i, location in enumerate(trip_data['locations'], start=1):
        builder.row(InlineKeyboardButton(text=f"{i}. {location['name']}",
                                         callback_data=f'show_car_rent_{location["id"]}'))
...
@router.callback_query(F.data.startswith('show_car_rent_'))
async def show_car_rent(callback: CallbackQuery,
                                  state: FSMContext,
                                  backend: BackendService,
