...
@router.callback_query(F.data.startswith('bar_action_'))
async def bar_action_tab(callback: CallbackQuery,
                         state: FSMContext,
    ...
    for i, location in enumerate(trip_data['locations'], start=1):
        builder.row(InlineKeyboardButton(text=f"{i}. {location['name']}",
                                         callback_data=f'bar_show_{location["id"]}'))
...
@router.callback_query(F.data.startswith('bar_show_'))
async def show_bars(callback: CallbackQuery,
                    state: FSMContext,
                    backend: BackendService,
