...
async def food_command(message: Message, state: FSMContext):
...
@router.message(OrderFood.choosing_food_name,
                F.text.lower().in_(available_food_names))
...
async def food_chosen_incorrectly(message: Message):
    await message.answer(text='Такого блюда нет. Попробуйте еще раз',
                         reply_markup=make_row_keyboard(*available_food_names))
